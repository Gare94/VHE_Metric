---
title: "Zinc Supply" Fortification st at status quo
author: "Gareth Osman"
date: "2025-04-07"
...
5. Merge
```{r}
#Merged DF with all dietary znriables
...
```{r}
#MN supply/AFE
ihs5.zn$zn.afe <- ihs5.zn$zn.supply/ihs5.zn$hh.afe
#AFE inadequacy
ihs5.zn <- ihs5.zn %>% mutate(zn.afe.inad = ifelse(zn.afe <10.2, "Inadequate", "Adequate"))
```
...
```{r}
#MN supply/AFE
ihs5.zn$zn.vuln <- ihs5.zn$zn.supply/ihs5.zn$vulnerable_equiv_zn

#Vulnerable inadequacy
ihs5.zn <- ihs5.zn %>% filter(!is.na(zn.vuln))
ihs5.zn$zn.vuln.inad <- NA

for (i in 1:nrow(ihs5.zn)) {
  if(ihs5.zn$zn.vuln[i] < ihs5.zn$vulnerable_equiv_H_AR_zn[i]){
    ihs5.zn$zn.vuln.inad[i] <- 'Inadequate'
  } else {
    ihs5.zn$zn.vuln.inad[i] <- 'Adequate'
  }
}
...
```{r}
write.csv(ihs5.zn, here::here ("paper_VHE/vuln.group/supply.Mn/results/zn.vuln.csv"), row.names = FALSE)
